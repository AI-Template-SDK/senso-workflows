	...
	fn, err := inngestgo.CreateFunction(
		...
		func(ctx context.Context, input inngestgo.Input[NetworkReevalProcessEvent]) (any, error) {
			...
			nameVariationsResult, err := step.Run(ctx, "generate-name-variations", func(ctx context.Context) (interface{}, error) {
				fmt.Printf("[ProcessNetworkReeval] Step 2.5: Generating name variations for org: %s\n", orgName)
				...
				fmt.Printf("[ProcessNetworkReeval] ✅ Generated %d name variations\n", len(variations))
			...
			// Step 3: Process each question run individually with cleanup (and pre-generated name variations)
