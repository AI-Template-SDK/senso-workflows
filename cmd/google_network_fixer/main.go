...
func createNetworkBatch(ctx context.Context, repos *services.RepositoryManager, networkUUID uuid.UUID, totalQuestions int) (*models.QuestionRunBatch, error) {
	...
	b := &models.QuestionRunBatch{
		...
		BatchType:          "google_network_fixer",
		Status:             "running",
...
func main() {
	var (
		...
		concurrency = flag.Int("concurrency", 3, "number of concurrent AI Overview calls/inserts per network (bounded)")
		...
		writeModel  = flag.String("write-model", "aioverview", "network model name (or substring) to backfill into question_runs.run_model (e.g. 'aioverview')")
	)
	flag.Parse()
	...
	if !*dryRun {
		if strings.TrimSpace(cfg.BrightDataSERPAPIKey) == "" {
			log.Fatalf("BRIGHTDATA_SERP_API_KEY must be set in .env for live runs (AI Overview via BrightData SERP API)")
		}
		provider = services.NewAIOverviewProvider(cfg, "aioverview", services.NewCostService())
	}
	...
	log.Printf("[google_network_fixer] networks=%d dry_run=%t concurrency=%d write_model=%s", len(networkIDs), *dryRun, *concurrency, *writeModel)
	if *dryRun {
		log.Printf("[google_network_fixer] DRY RUN MODE: no DB writes, no AI Overview calls will be made")
		log.Printf("[google_network_fixer] To execute for real: go run ./cmd/google_network_fixer --dry-run=false --write-model %s --concurrency %d", *writeModel, *concurrency)
	}

	todayStart := utcTodayStart(time.Now())
	log.Printf("[google_network_fixer] todayStart(UTC)=%s", todayStart.Format(time.RFC3339))

	for idx, networkID := range networkIDs {
		log.Printf("[google_network_fixer] (%d/%d) network=%s", idx+1, len(networkIDs), networkID)

		networkUUID, err := uuid.Parse(networkID)
		if err != nil {
			log.Printf("[google_network_fixer] network=%s invalid uuid: %v", networkID, err)
			continue
		}
		...
		if err != nil {
			log.Printf("[google_network_fixer] network=%s ERROR get network models: %v", networkID, err)
			continue
		}
		if len(modelNames) == 0 {
			log.Printf("[google_network_fixer] network=%s skip (no network models configured; not using fallback defaults)", networkID)
			continue
		}
		...
		if len(writeModels) == 0 {
			log.Printf("[google_network_fixer] network=%s skip (no network model matching %q configured)", networkID, *writeModel)
			continue
		}

		networkQuestions, networkLocations, err := loadNetworkQuestionsAndLocations(ctx, repos, networkUUID)
		if err != nil {
			log.Printf("[google_network_fixer] network=%s ERROR load questions/locations: %v", networkID, err)
			continue
		}
		...
		if err != nil {
			log.Printf("[google_network_fixer] network=%s ERROR finding today's batch: %v", networkID, err)
			continue
		}
		isExisting := batch != nil
		if !isExisting {
			if *dryRun {
				log.Printf("[google_network_fixer] network=%s DRY RUN would create today's batch (type=google_network_fixer total_questions=%d)", networkID, totalQuestions)
			} else {
				createdBatch, err := createNetworkBatch(ctx, repos, networkUUID, totalQuestions)
				if err != nil {
					log.Printf("[google_network_fixer] network=%s ERROR creating today's batch: %v", networkID, err)
					continue
				}
				batch = createdBatch
			}
		...
		log.Printf("[google_network_fixer] network=%s batch=%s (existing=%t status=%s)", networkID, batchID, isExisting, batchStatus)
		...
		if len(jobs) == 0 {
			log.Printf("[google_network_fixer] network=%s done (no missing runs) skipped_existing=%d", networkID, skippedExisting)
			continue
		}
		log.Printf("[google_network_fixer] network=%s missing_jobs=%d skipped_existing=%d (executing with concurrency=%d)", networkID, len(jobs), skippedExisting, *concurrency)
		...
		worker := func() {
			...
			for job := range jobsCh {
				...
				aiResp, err := provider.RunQuestion(ctx, job.qText, true, loc)
				if err != nil {
		...
		for res := range resultsCh {
			if res.failed {
				failedCount++
				log.Printf("[google_network_fixer] network=%s ERROR job question=%s model=%s location=%s: %v",
			...
			if res.created {
				...
				totalCost += res.cost
				if *dryRun {
					log.Printf("[google_network_fixer] DRY RUN would insert run question=%s model=%s location=%s", res.job.qID, res.job.writeModel, res.job.country)
				}
			}
		}

		log.Printf("[google_network_fixer] network=%s done created=%d skipped_existing=%d failed=%d total_cost=%.6f", networkID, createdCount, skippedExisting, failedCount, totalCost)
	}

	log.Printf("[google_network_fixer] done")
}
